...
rockcraft_projects = glob.glob("rockcraft*.yaml")
...
for rock_project in rockcraft_projects:
    ...
    publish_to = [
        # Docker Hub
        f"ubuntu/{rock_name}:{tag_name}",
        # ACR
        f"ubuntu.azurecr.io/{rock_name}:{tag_name}",
    ]

    logging.info(" ----- PUBLISHING TO -----")
    for repo in publish_to:
        raw_cmd = str(
            "skopeo --insecure-policy copy --preserve-digests --multi-arch all "
            f"oci:{oci_name} docker://{repo}"
        )
        logging.info(f"  --> {repo}")

        subprocess.check_output(raw_cmd.split())
